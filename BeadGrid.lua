...
local function applyVerticalDeskew(sourceImage, slope)
  if not slope or math.abs(slope) < 0.0001 then
    return sourceImage
  end
  ...
  for x = 0, sourceImage.width - 1 do
    local shift = roundNearest(x * slope)
    shifts[x + 1] = shift
  ...
  local outputHeight = sourceImage.height + (maxShift - minShift)
  local output = Image(sourceImage.width, outputHeight, sourceImage.colorMode)
  output:clear(0)

  for y = 0, sourceImage.height - 1 do
    for x = 0, sourceImage.width - 1 do
      local pixel = sourceImage:getPixel(x, y)
      if pc.rgbaA(pixel) > 0 then
        local destY = y - shifts[x + 1] + maxShift
        output:putPixel(x, destY, pixel)
      end
    end
  end
...
local function buildPattern(sourceImage, bounds, targetWidth, targetHeight, palette)
  local cells = {}
  local counts = {}
...
local function shouldDrawAxisLabel(index, total, step)
  return index == 0 or index == total or (index % step) == 0
end
...
local function lineColorForIndex(index, normalColor, majorColor)
  if isMajorGridLine(index) then
    return majorColor
  end
  return normalColor
end
...
local function drawAxisRaster(image, pattern, gridX, gridY, cellSize, axis, color)
  for line = 0, pattern.width do
    if shouldDrawAxisLabel(line, pattern.width, axis.step) then
      local label = tostring(line)
      if line == 0 then
        label = "0"
      end
      local labelWidth = measureBitmapText(label, axis.scale)
      local labelX = gridX + line * cellSize - math.floor(labelWidth / 2)
  ...
  for line = 0, pattern.height do
    if shouldDrawAxisLabel(line, pattern.height, axis.step) then
      local label = tostring(line)
      if line == 0 then
        label = "0"
      end
      ...
      local labelY = gridY + line * cellSize - math.floor((5 * axis.scale) / 2)
      safeDrawText(image, label, color, labelX, labelY, axis.scale)
    end
  end
end

local function drawCellGridRaster(image, pattern, gridX, gridY, cellSize, normalColor, majorColor, borderColor)
  for y = 0, pattern.height - 1 do
    for x = 0, pattern.width - 1 do
      ...
      if entry then
        ...
        local topColor = lineColorForIndex(y, normalColor, majorColor)
        local leftColor = lineColorForIndex(x, normalColor, majorColor)
        local rightColor = lineColorForIndex(x + 1, normalColor, majorColor)
        local bottomColor = lineColorForIndex(y + 1, normalColor, majorColor)

        if normalColor then
          drawFilledRect(image, px, py, cellSize, 1, topColor)
          drawFilledRect(image, px, py, 1, cellSize, leftColor)
          drawFilledRect(image, px + cellSize - 1, py, 1, cellSize, rightColor)
          drawFilledRect(image, px, py + cellSize - 1, cellSize, 1, bottomColor)
        elseif borderColor then
          drawRectOutline(image, px, py, cellSize, cellSize, borderColor)
        end
      end
    end
  end
...
local function makePreviewImage(pattern, options)
  ...
  local contentPixelWidth = gridPixelWidth + axis.leftPad + axis.rightPad
  local contentPixelHeight = gridPixelHeight + axis.topPad + axis.bottomPad
  ...
  for y = 0, pattern.height - 1 do
    for x = 0, pattern.width - 1 do
      ...
      if entry then
        local px = gridX + x * cellSize
        local py = gridY + y * cellSize
        drawFilledRect(image, px, py, cellSize, cellSize, entry.rgba)
  ...
  if options.showGrid then
    drawCellGridRaster(image, pattern, gridX, gridY, cellSize, gridColor, majorGridColor, nil)
  else
    drawCellGridRaster(image, pattern, gridX, gridY, cellSize, nil, nil, black)
  end
  ...
  for y = 0, pattern.height - 1 do
    for x = 0, pattern.width - 1 do
      ...
      if entry then
        ...
        if options.showGrid then
          local topStroke = lineColorForIndex(y, "#C8C8C8", "#6E8CB4")
          local leftStroke = lineColorForIndex(x, "#C8C8C8", "#6E8CB4")
          local rightStroke = lineColorForIndex(x + 1, "#C8C8C8", "#6E8CB4")
          local bottomStroke = lineColorForIndex(y + 1, "#C8C8C8", "#6E8CB4")
          parts[#parts + 1] = string.format(
            '  <line x1="%d" y1="%d" x2="%d" y2="%d" stroke="%s" stroke-width="0.6"/>\n',
            cellX, cellY, cellX + cellSize, cellY, topStroke
          )
          parts[#parts + 1] = string.format(
            '  <line x1="%d" y1="%d" x2="%d" y2="%d" stroke="%s" stroke-width="0.6"/>\n',
            cellX, cellY, cellX, cellY + cellSize, leftStroke
          )
          parts[#parts + 1] = string.format(
            '  <line x1="%d" y1="%d" x2="%d" y2="%d" stroke="%s" stroke-width="0.6"/>\n',
            cellX + cellSize, cellY, cellX + cellSize, cellY + cellSize, rightStroke
          )
          parts[#parts + 1] = string.format(
            '  <line x1="%d" y1="%d" x2="%d" y2="%d" stroke="%s" stroke-width="0.6"/>\n',
            cellX, cellY + cellSize, cellX + cellSize, cellY + cellSize, bottomStroke
          )
        else
          parts[#parts + 1] = string.format(
            '  <rect x="%d" y="%d" width="%d" height="%d" fill="none" stroke="#111111" stroke-width="0.6"/>\n',
...
local function directoryOfPath(filePath)
  local normalized = filePath:gsub("\\", "/")
  return normalized:match("^(.*)/[^/]+$") or "."
end

local function joinPath(dirPath, fileName)
...
local function makeQuantizedImage(sourceImage, palette)
  local output = Image(sourceImage.width, sourceImage.height, sourceImage.colorMode)
  output:clear(0)
  ...
  for y = 0, sourceImage.height - 1 do
    for x = 0, sourceImage.width - 1 do
      ...
      if alpha > 0 then
        local entry = nearestPaletteColor(pixel, palette)
        if entry then
          output:putPixel(x, y, pc.rgba(entry.r, entry.g, entry.b, alpha))
        end
      end
    end
  end
...
local function saveImageAsAseprite(image, filePath)
  local originalSprite = app.activeSprite
  local tempSprite = nil
  local ok, err = pcall(function()
    tempSprite = Sprite(image.width, image.height, ColorMode.RGB)
    local layer = tempSprite.layers[1]
    tempSprite:newCel(layer, 1, image, Point(0, 0))
    tempSprite:saveAs(filePath)
  end)
  ...
  if tempSprite and tempSprite.close then
    pcall(function()
      tempSprite:close()
    end)
  end
  ...
  if not ok then
    return false, err or "Unable to save Aseprite file."
  end
  return true
end
...
local function runExporter()
  ...
  local canvasBounds = spriteBounds(sprite)
  local sourceImage = flattenSpriteFrame(sprite, frameNumber)
  local bounds = findOpaqueBounds(sourceImage)
  if not bounds then
    app.alert("No visible pixels found in the current frame.")
    return
  end
  ...
  dialog:label {
    ...
    label = "Source",
    text = string.format(
      "%dx%d canvas  ->  %dx%d visible  frame:%d",
      canvasBounds.width,
      canvasBounds.height,
      bounds.width,
      bounds.height,
  ...
  local deskewSlope = 0
  if data.deskewMode == "Iso Up" then
    deskewSlope = 0.5
  elseif data.deskewMode == "Iso Down" then
    deskewSlope = -0.5
  end
  local processedImage = applyVerticalDeskew(sourceImage, deskewSlope)
  bounds = findOpaqueBounds(processedImage)
  if not bounds then
    app.alert("No visible pixels found after deskew.")
    return
  end
  ...
  local asepriteImage = makeQuantizedImage(processedImage, selectedPalette)
  local asepritePath = joinPath(directoryOfPath(pngPath), baseName .. "_beadgrid.aseprite")

  local messages = {
    string.format("Pattern generated: %dx%d", width, height),
    string.format("Filled cells: %d", pattern.filled),
    string.format("Deskew: %s", data.deskewMode or "None")
  }
  ...
  local asepriteOk, asepriteErr = saveImageAsAseprite(asepriteImage, asepritePath)
  if not asepriteOk then
    app.alert("Aseprite export failed: " .. tostring(asepriteErr))
    return
  end

  messages[#messages + 1] = "PNG saved."
  messages[#messages + 1] = "SVG saved."
  messages[#messages + 1] = "Aseprite saved."
